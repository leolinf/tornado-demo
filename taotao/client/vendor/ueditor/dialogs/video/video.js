...
(function(){
    ...
    function insertSingle1(){
        var  url = $G("videoUrl").value.replace(/^\s*|\s*$/ig,""),
                width = $G("videoWidth").value ? $G("videoWidth").value : 500,
                height = $G("videoHeight").value ? $G("videoHeight").value : 375,
                scroll = 0, // $G("scroll"),
                frameborder = 0, // $G("frameborder"),
                float = findFocus("videoFloat","name"), // $G("align").value,
        ...
        newIframe.setAttribute("data-src",/http:\/\/|https:\/\//ig.test(url) ? url : "http://"+url);
        newIframe.setAttribute("class", "video_iframe");
        newIframe.setAttribute("style", "z-index: 1;");
    ...
    function convert_url(url){
        ...
        url = utils.trim(url)
            ...
            */
            ...
            .replace(/v\.qq\.com\/.+\/([^&]+)\.html$/i, "v.qq.com/iframe/player.html?vid=$1&auto=0")
            .replace(/weishi\.qq\.com\/t\/([0-9]+).*/i, "z.weishi.com/weixin/player.html?msgid=$1&auto=0");
    ...
    function createPreviewVideo(url){
        ...
        */
        ...
        newIframe.setAttribute("class", "previewVideo");
   
        div = editor.document.createElement("div");
        div.appendChild($('<div class="previewMsg"><span>仅支持腾讯视频和微视频</span></div>').get(0));
        div.appendChild(newIframe);
        $G("preview").innerHTML = div.innerHTML;
